...
defaults to "word_metrics_"
  -seperator    string  Record seperator, defaults to "KPBLOCK\r\n"
...
$line =~ s/\s+/ /g;
#print "LINE: $line\n";
my ($loose_time,$host,$request,$status) = split(/|/,$line,4); 

my $t;
eval{
$loose_time =~ s/[^[:ascii:]]//g;
$t = Time::Piece->strptime($loose_time,"%s");
};
if ( my $err = $@ ) {
